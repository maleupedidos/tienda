 ...
 *
 *   node _tools/verificar-sugerencia-carne.js [ancho]      (390 por defecto)
 *
 ...
 *      ofrece la siguiente;
 *   5. solo carne en el carrito, todo agotado, y Pilar: no sugiere;
...
const idPMu = Number((APP.match(/\{\s*id:(\d+),\s*cat:"[^"]+",\s*nombre:"Pizza Muzzarella"/) || [])[1]);
...
async function main() {
  ...
  try {
    ...
    const sug = async () => JSON.parse(await ev('(function () {' +
      ...
      'var filas = s ? [].map.call(s.querySelectorAll(".sug-carne-fila"), function (f) {' +
      '  var b = f.querySelector(".sug-carne-btn");' +
      ...
      'var ver = s && s.querySelector(".sug-carne-ver");' +
      'var body = document.getElementById("cart-body");' +
      'return JSON.stringify({ vis: vis, filas: filas, ver: ver ? Math.round(ver.getBoundingClientRect().height) : 0,' +
    ...
    await abrirCarrito();
    s = await sug();
    chk(s.vis, 'aparece la sugerencia');
    ...
    /* ── 3. + SUMAR ─────────────────────────────────────────────────── */
    console.log('\n' + DIM + '== + Sumar ==' + RST);
    const t = await tocar('#cart-sug .sug-carne-fila .sug-carne-btn');
    ...
    await tocar('#cart-sug .sug-carne-ver');
    await dormir(1200);
    ...
    await abrirCarrito();
    s = await sug();
    chk(!s.vis && (await ev('cartCount()')) > 0 && (await ev('piezasAgrupadas().length')) === 1, 'con solo carne en el carrito no sugiere');
    await cerrarCarrito();
